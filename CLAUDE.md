...
## PathWise — Project Context

This is the project ruflo was installed into. Where the generic guidance above
conflicts with this section, **this section wins**.

### Stack

- **TanStack Start** (`@tanstack/react-start`) + **TanStack Router** (file-based)
  + **React Query** — React 19, Vite 7
- **Tailwind v4** (`@tailwindcss/vite`) + **shadcn/ui** on Radix primitives
- **Supabase** (`@supabase/supabase-js`) with **Lovable Cloud auth**
  (`@lovable.dev/cloud-auth-js`)
- **Cloudflare** deploy target (`@cloudflare/vite-plugin`, `wrangler.jsonc`);
  `vercel.json` also present
- **Zod** + react-hook-form for validation

### Layout — do NOT apply the generic `/src /tests /docs /config /scripts` rule

| Path | Holds |
|---|---|
| `src/routes/` | File-based routes; `routeTree.gen.ts` is **generated — never hand-edit** |
| `src/pathwise/` | Domain code — `matching.ts`, `roadmap-gen.ts`, `progress.ts`, `scheduling.ts`, `roles.ts`, `auth.tsx`, `store.ts` + feature components |
| `src/components/` | Shared UI (shadcn) |
| `src/integrations/` | Supabase client and external wiring |
| `src/hooks/`, `src/lib/` | Shared hooks and utilities |
| `tests/` | `node --test` suites (`*.test.ts`) — currently only `roles.test.ts` |
...
- **RLS is the historical pain point.** The repo root carries a long trail of
  `RLS_FIX_*.md` / `ROADMAP_*.md` postmortems. Treat any change touching
  Supabase policies, `roles.ts`, or `RoleGate`/`RequireOnboarding` as
  security-sensitive: read the existing policy first, and prefer the
  `security-architect` / `auditor` routing above.
- **Test coverage is thin** (one suite). "ALWAYS run tests" is necessary but far
  from sufficient here — verify behaviour in the app, don't lean on a green run.
- **Never hand-edit `src/routeTree.gen.ts`** — regenerate it via the router plugin.
- **Auth is Lovable Cloud, not vanilla Supabase auth.** Don't assume
  `supabase.auth` patterns transfer.
- Root-level `*.md` clutter already violates the "never save to root" rule above.
  Don't add to it; new docs belong in `docs/`.

### Swarm proportionality

PathWise is ~138 TS files with a single developer. Per the "When to Swarm" table
above, most work here lands in the **NO** column — single-file edits, small
fixes, config. Reserve multi-agent swarms for genuine cross-cutting work
(auth/RLS changes, roadmap-generation refactors, route-tree restructuring).
