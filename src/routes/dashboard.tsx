...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      { title: "Tutor Dashboard — PathWise" },
      { name: "description", content: "Earnings, students, sessions and growth — all in one view." },
      { property: "og:title", content: "Tutor Dashboard — PathWise" },
      { property: "og:description", content: "Manage your students, sessions, courses, and earnings on PathWise." },
...
function GuardedDashboard() {
  return <RequireOnboarding><DashboardPage /></RequireOnboarding>;
}

// ---------- Page ----------
function DashboardPage() {
  const { isLoggedIn, role, openLogin, loading, user, profile } = useAuth();
  const navigate = useNavigate();
  const tutorId = role === "tutor" || role === "both" ? user?.id : undefined;

  useEffect(() => {
    if (loading) return;
    if (!isLoggedIn) { openLogin(); return; }
    if (role && role !== "tutor" && role !== "both") navigate({ to: "/" });
  ...
  return (
    <DashboardShell
      ...
      user={{
        ...
        subtitle: tutorProfile?.bio ?? undefined,
      }}
    >
        {/* Welcome */}
        <div className="mt-4">
...
// ---------- Earnings breakdown side widget ----------
