...
const DAY_INDEX = [1, 2, 3, 4, 5, 6, 0]; // map column -> JS day_of_week
const HOURS = Array.from({ length: 24 }, (_, i) => i);
const BUFFER_OPTIONS = [0, 15, 30, 60];

/** Slot state per (day, hour) cell. */
...
function AvailabilityPage() {
  ...
  function paint(day: number, hour: number, mode: CellState) {
    const key = `${day}-${hour}`;
    if (bookedSlots.has(key)) return; // can't override a booked slot
  ...
  /** Convert grid back into contiguous (day, start_hour, end_hour, is_blocked) ranges. */
